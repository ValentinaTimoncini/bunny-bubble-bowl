...
async function x_build(x_currhash){
    ...
    let x_content_response = await fetch('/views/'+x_site.x_view)
    ...
    if(document.getElementById("foot").innerHTML.length === 0) {
        let x_footer_response = await fetch('/components/footer.html')
    ...
    x_script.src = "/src/js/"+x_site.x_script;
    document.body.append(x_script);

    x_stop_laoding()
}
async function x_start_laoding(){
    let x_loader_response = await fetch('/components/loader.html')
